...
class CertificateUpload extends File
{
    ...
    public function beforeSave()
    {
        ...
        if ((isset($value['name']) && isset($file['name'])) || isset($value['delete'])) {
            ...
            if (!empty($file)) {
                $uploadDir = $this->filesystem->getDirectoryWrite(DirectoryList::VAR_DIR)->getAbsolutePath('certificates/');
                ...
                if (!$directoryWrite->isExist('certificates/')) {
                    try {
                        $directoryWrite->create('certificates');
                    } catch (\Exception $e) {
    ...
    public function getOldCertificatePath()
    {
        ...
        if ($configValue) {
            $uploadDir = $this->_filesystem->getDirectoryWrite(DirectoryList::VAR_DIR)->getAbsolutePath('certificates/');
